...
def get_recensement(relative_path=""):

    # Loading data
    recensement = pd.read_excel(io=f"{relative_path}1832_v4.xlsx", na_values=["·","?"])
    classes = pd.read_excel(io=f"{relative_path}histoire urbaine digitale classification metiers.xlsx", sheet_name="data",  na_values="·")
    rues = pd.read_csv(f"{relative_path}nom_rues_et_coor.csv", sep=",")
    regions = pd.read_excel(f"{relative_path}histoire urbaine digitale classification origines.xlsx")
